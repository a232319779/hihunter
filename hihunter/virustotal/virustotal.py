...
class VirusTotal(object):
    ...
    def api_key_statics(self):
        ...
        """
        try:
            url = "https://www.virustotal.com/api/v3/users/{user}/overall_quotas".format(user=self.api_key)
            ...
            data['api_requests_used'] = self.__get_quota_data__(res_json, 'api_requests_monthly')
            data['api_requests_total'] = self.__get_quota_data__(res_json, 'api_requests_daily', False) * month_len
            data['api_requests_used_ratio'] = data['api_requests_used'] / data['api_requests_total']
            data['api_requests_hourly_used'] = self.__get_quota_data__(res_json, 'api_requests_hourly')
    ...
    def filter(self, query, limit=50, descriptors_only=False, cursor=''):
        ...
        """
        try:
            url = "https://www.virustotal.com/api/v3/intelligence/search?query={query}&limit={limit}&descriptors_only={descriptors_only}&cursor={cursor}".format(query=query, limit=limit, descriptors_only=descriptors_only, cursor=cursor)
            ...
            for vrd in vt_return_dats:
                ...
                one_data['positive'] = vrd.get('attributes', {}).get('last_analysis_stats', {}).get('malicious', -1)
                one_data['times_submitted'] = vrd.get('attributes', {}).get('times_submitted', -1)
                one_data['unique_sources'] = vrd.get('attributes', {}).get('unique_sources', -1)
                one_data['type'] = vrd.get('attributes', {}).get('type_description', '')
                one_data['tags'] = ','.join(vrd.get('attributes', {}).get('tags', []))
                one_data['suggested_threat_label'] = vrd.get('attributes', {}).get('popular_threat_classification', {}).get('suggested_threat_label', '')
                one_data['first_submission_date'] = vrd.get('attributes', {}).get('first_submission_date', -1)
    ...
    def download(self, hash, download_path='./'):
        ...
        data = {'hash': hash}
        try:
            url = "https://www.virustotal.com/api/v3/files/{hash}/download".format(hash=hash)
            ...
            if response.content:
                ...
                return return_data(10000, 'download success', data)
            return return_data(10101, 'download failed', data)
        except Exception as e:
